...
class ROSDataBuffer(Node):
    def __init__(self):
        ...
        self.declare_parameter("pose_topic", "/log_player/out/pose")
        self.declare_parameter("input_topic", "")
        self.declare_parameter("raw_topic", "/data_logging_msg")
        self.declare_parameter("raw_layout", "auto")
        self.declare_parameter("force_gt_topic", "/log_player/online/force_estimate")
        self.declare_parameter("mob_1st_topic", "/log_player/offline/drone_external_force_mob")
        self.declare_parameter("mob_2nd_topic", "/log_player/offline/drone_external_force_mob_2nd")
        self.declare_parameter("mob_consistency_topic", "/log_player/offline/drone_external_force_mob_2nd_tau")
        self.declare_parameter("mob_consistency_terms_topic", "/log_player/offline/drone_external_force_mob_2nd_tau_terms")
        self.declare_parameter("mob_consistency_match_topic", "/log_player/offline/drone_external_force_mob_2nd_tau_consistency")
        ...
        self.declare_parameter("mass", 0.0393)
        self.declare_parameter("g", 9.81)
        self.declare_parameter("motor.force_source", "scaled")
        self.declare_parameter("motor.arm_length", 0.046)
        self.declare_parameter("motor.thrust_to_torque", 0.006)
        ...
        self.raw_layout = str(self.get_parameter("raw_layout").value)
        self.motor_force_source = str(self.get_parameter("motor.force_source").value)
        self.motor_arm_length = float(self.get_parameter("motor.arm_length").value)
        self.motor_thrust_to_torque = float(self.get_parameter("motor.thrust_to_torque").value)
    ...
    def _resolved_layout(self, n: int) -> str:
        if self.raw_layout in ("specific49", "specific50"):
            return self.raw_layout
        return "specific50" if n == 50 else "specific49"

    def _decode_motor_forces(self, data) -> np.ndarray | None:
        layout = self._resolved_layout(len(data))
        if layout == "specific49":
            if len(data) < 39:
                return None
            base = 31 if self.motor_force_source == "raw" else 35
            values = np.array(data[base:base + 4], dtype=float)
            if values.size != 4 or not np.all(np.isfinite(values)):
                return None
            return values
        return None

    def _compute_input_wrench_from_motors(self, motor_forces: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        f1, f2, f3, f4 = [float(v) for v in motor_forces]
        arm = self.motor_arm_length * 0.7071067811865476
        body_force = np.array([0.0, 0.0, f1 + f2 + f3 + f4], dtype=float)
        body_torque = np.array([
            -arm * f1 - arm * f2 + arm * f3 + arm * f4,
            -arm * f1 + arm * f2 + arm * f3 - arm * f4,
            -self.motor_thrust_to_torque * f1 + self.motor_thrust_to_torque * f2
            - self.motor_thrust_to_torque * f3 + self.motor_thrust_to_torque * f4,
        ], dtype=float)
    ...
    def cb_raw(self, msg: Float64MultiArray) -> None:
        motor_forces = self._decode_motor_forces(msg.data)
        if motor_forces is None:
            return

        body_force, body_torque = self._compute_input_wrench_from_motors(motor_forces)
        with self.lock:
            self.ctrl_force_world = self.rot_body_to_world @ body_force
            self.ctrl_tau_world = self.rot_body_to_world @ body_torque
            self.input_tau_fz = np.array([
                body_torque[0],
                body_torque[1],
                body_torque[2],
                body_force[2],
...
class PlotWindow(QMainWindow):
    def __init__(self, rosbuf: ROSDataBuffer):
        ...
        root.addWidget(self._build_residual_panel(), stretch=3)
        root.addWidget(self._build_force_compare_panel(), stretch=3)
    ...
    def _build_force_panel(self) -> QWidget:
        ...
        self.force_curves = {}
        colors = {
            "input": pg.mkPen((20, 20, 20), width=2, style=Qt.DashLine),
            "mob": pg.mkPen((220, 70, 45), width=2),
        }
        ...
        for i, axis in enumerate(AXES):
            ...
            self.force_curves[f"ctrl_f{axis}"] = plot.plot(name="R_B * F input", pen=colors["input"])
            self.force_curves[f"mobc_f{axis}"] = plot.plot(name="MOB force", pen=colors["mob"])
    ...
    def _build_torque_panel(self) -> QWidget:
        ...
        self.torque_curves = {}
        colors = {
            "ctrl": pg.mkPen((20, 20, 20), width=2, style=Qt.DashLine),
            "mob": pg.mkPen((220, 70, 45), width=2),
        }
        ...
        for i, axis in enumerate(AXES):
            ...
            self.torque_curves[f"ctrl_tau_t{axis}"] = plot.plot(name="Torque input", pen=colors["ctrl"])
            self.torque_curves[f"tauc_t{axis}"] = plot.plot(name="MOB torque", pen=colors["mob"])
            grid.addWidget(plot, 0, i)

        frame.layout().addLayout(grid)
        return frame

    def _build_residual_panel(self) -> QWidget:
        frame = self._make_group_frame("Residual: MOB tau hat vs r x MOB F")
        grid = QGridLayout()
        grid.setContentsMargins(0, 0, 0, 0)
        grid.setHorizontalSpacing(6)
        grid.setVerticalSpacing(6)
        ...
        for i, axis in enumerate(AXES):
            ...
            self.residual_plots.append(plot)
            self.residual_curves[f"tauhat_t{axis}"] = plot.plot(
                name="tau_hat_ext", pen=colors["tauhat"])
            self.residual_curves[f"rxf_mobc_t{axis}"] = plot.plot(
                name="r x MOB F", pen=colors["rxf"])
            grid.addWidget(plot, 0, i)

        frame.layout().addLayout(grid)
        return frame

    def _build_force_compare_panel(self) -> QWidget:
        frame = self._make_group_frame("Pure MOB F vs Consistency Residual MOB F")
        grid = QGridLayout()
        grid.setContentsMargins(0, 0, 0, 0)
        grid.setHorizontalSpacing(6)
        grid.setVerticalSpacing(6)

        self.force_compare_plots = []
        self.force_compare_curves = {}
        colors = {
            "pure": pg.mkPen((40, 90, 220), width=2),
            "consistency": pg.mkPen((220, 70, 45), width=2),
        }
        ...
        for i, axis in enumerate(AXES):
            ...
            self.force_compare_plots.append(plot)
            self.force_compare_curves[f"mob2_f{axis}"] = plot.plot(name="pure MOB F", pen=colors["pure"])
            self.force_compare_curves[f"mobc_f{axis}"] = plot.plot(
                name="consistency MOB F", pen=colors["consistency"])
            grid.addWidget(plot, 0, i)
    ...
    def _mob_force_display(self, values: np.ndarray, axis: str) -> np.ndarray:
        out = np.array(values, dtype=float, copy=True)
        if axis in ("x", "y"):
            out = -out
        return out

    def _mob_torque_display(self, values: np.ndarray, axis: str) -> np.ndarray:
        out = np.array(values, dtype=float, copy=True)
        if axis == "y":
            out = -out
        return out
    ...
    def update_all(self) -> None:
        ...
        for axis in AXES:
            self.force_curves[f"ctrl_f{axis}"].setData(t_win, self._force_axis(arr[f"ctrl_f{axis}"][mask]))
            self.force_curves[f"mobc_f{axis}"].setData(
                t_win, self._force_axis(self._mob_force_display(arr[f"mobc_f{axis}"][mask], axis)))

        for axis in AXES:
            self.torque_curves[f"ctrl_tau_t{axis}"].setData(t_win, self._torque_axis(arr[f"ctrl_tau_t{axis}"][mask]))
            self.torque_curves[f"tauc_t{axis}"].setData(
                t_win, self._torque_axis(self._mob_torque_display(arr[f"tauc_t{axis}"][mask], axis)))

            self.residual_curves[f"tauhat_t{axis}"].setData(
                t_win, self._torque_axis(self._mob_torque_display(arr[f"tauhat_t{axis}"][mask], axis)))
            self.residual_curves[f"rxf_mobc_t{axis}"].setData(
                t_win, self._torque_axis(self._mob_torque_display(arr[f"rxf_mobc_t{axis}"][mask], axis)))

            self.force_compare_curves[f"mob2_f{axis}"].setData(
                t_win, self._force_axis(self._mob_force_display(arr[f"mob2_f{axis}"][mask], axis)))
            self.force_compare_curves[f"mobc_f{axis}"].setData(
                t_win, self._force_axis(self._mob_force_display(arr[f"mobc_f{axis}"][mask], axis)))
        ...
        self.info_label.setText(
            ...
            f"|F_mob|={self._fmt_force_axis(latest['mobc_f_norm'])} N(x1e-3/x1e-06)   "
            f"|F_pure|={self._fmt_force_axis(latest['mob2_f_norm'])} N(x1e-3/x1e-06)   "
            f"|tauhat-rxF|={self._fmt_torque_axis(latest['e_tau_norm'])} Nm(x1e-3/x1e-09)   "
