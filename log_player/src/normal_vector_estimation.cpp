...
public:
  ...
  : Node("normal_vector_estimation")
  {
    ...
    force_positive_x_hemisphere_ = this->declare_parameter<bool>(
      "force_positive_x_hemisphere", true);
    ...
    consistency_match_topic_ = this->declare_parameter<std::string>(
      "consistency_match_topic", "/log_player/offline/drone_external_force_mob_2nd_tau_consistency");

    use_vel_mode_topic_ = this->declare_parameter<std::string>(
      "use_vel_mode_topic", "/log_player/offline/use_vel_mode");
    ...
    raw_mob_wrench_topic_ = this->declare_parameter<std::string>(
      "raw_mob_wrench_topic", "/log_player/offline/ee_applied_mob_2nd");
    ...
    force_based_epsilon_g_ = this->declare_parameter<double>(
      "normal_force_based.epsilon_g", 1.0e-3);
    force_based_epsilon_f_ = this->declare_parameter<double>(
      "normal_force_based.epsilon_f", 1.0e-3);
...
private:
  ...
  void enforceNormalHemisphere(ContactFrame & cf_out) const
  {
    if (force_positive_x_hemisphere_ && cf_out.n_w.x() < 0.0) {
      cf_out.n_w = -cf_out.n_w;
    }
  }
  ...
  std::string resolveForceObservationTopic(const std::string & source) const
  {
    ...
    {
      return "/log_player/online/force_estimate";
    }
    if (source == "mob") {
      return "/log_player/offline/ee_applied_mob";
    }
    if (source == "mob_2nd" || source == "mob2") {
      return "/log_player/offline/ee_applied_mob_2nd";
    }
    if (source == "mob_2nd_tau" || source == "mob_tau" || source == "consistency") {
      return "/log_player/offline/ee_applied_mob_2nd_tau";
    }
    if (source == "mob_2nd_tau_ke2" || source == "mob_tau_ke2" || source == "consistency_ke2") {
      return "/log_player/offline/ee_applied_mob_2nd_tau_ke2";
    }

    RCLCPP_WARN(
      this->get_logger(),
      "Unknown force_observation_source '%s'. Falling back to '/log_player/online/force_estimate'.",
      source.c_str());
    return "/log_player/online/force_estimate";
  }
    ...
    double dt)
  {
    ...
    const double vel_norm_sq = v_c_world.squaredNorm();
    Eigen::Vector3d w_s = Eigen::Vector3d::Zero();
    if (vel_norm > 1e-12) {
    ...
    const double gamma_v =
      vel_norm_sq / (vel_norm_sq + force_based_epsilon_g_ + 1e-12);

    Eigen::Vector3d n_f = Eigen::Vector3d::Zero();
    if (force_norm > force_based_epsilon_f_) {
    ...
    const Eigen::Matrix3d projector =
      ...
      (v_c_world * v_c_world.transpose()) / (vel_norm_sq + force_based_epsilon_g_ + 1e-12);
    const Eigen::Vector3d f_g = projector * n_f;
    const double corrected_force_norm = f_g.norm();

    Eigen::Vector3d n_alg = Eigen::Vector3d::Zero();
    if (corrected_force_norm > force_based_epsilon_f_) {
  ...
  {
    enforceNormalHemisphere(cf_out);
  ...
  bool force_positive_x_hemisphere_{true};
  ...
  double force_based_epsilon_g_{1.0e-3};
  double force_based_epsilon_f_{1.0e-3};
